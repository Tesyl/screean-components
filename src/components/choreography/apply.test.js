...
import { __resetNodeIds, scene, spawn, TRANSPARENT, World, } from 'screean';
...
const setupRunner = (component) => {
    const s = scene({ particleCount: 8 }, component);
    s.tick(0);
    const w = new World({ width: 100, height: 100 });
    w.addParticles(spawn({ n: 8, origin: { kind: 'point', x: 0, y: 0 }, color: TRANSPARENT }));
...
describe('applyDefaultChoreography', () => {
    it('wires button click to dissolve via the registry', () => {
        ...
        btn._component.handlers.onClick?.({});
        runner.tick(0);
    ...
    it('per-instance override beats registry for that event only', () => {
        ...
        expect(kickedIndices.length).toBeGreaterThan(0);
    });
    ...
    it('composes well: kick + dissolve via override', () => {
        ...
        runner.tick(0);
        const moved = particles.some((p, i) => p.vx !== before[i].vx || p.vy !== before[i].vy);
