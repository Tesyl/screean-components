...
const buildTaggedComponent = (): Component => {
...
const buildSceneAndParticles = (
  c: Component,
  particleCount = 12,
): { s: Scene; particles: Particle[] } => {
  ...
  return { s, particles: w.particles as Particle[] };
};
...
describe('groupWhere', () => {
  it('filters particles by predicate', () => {
    ...
    for (let i = 0; i < particles.length; i += 2) particles[i].life = 99;
