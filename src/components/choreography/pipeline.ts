...
type AtPlacedPipeline = Pipeline & { [AT_PLACED]?: true };

const isPipeline = (x: Effect | Pipeline): x is Pipeline =>
  'stages' in x && 'duration' in x;

const isAtPlaced = (p: Pipeline): boolean =>
  Boolean((p as AtPlacedPipeline)[AT_PLACED]);

const markAtPlaced = (p: Pipeline): Pipeline => {
  (p as AtPlacedPipeline)[AT_PLACED] = true;
  return p;
};
...
export const at = (offsetMs: number, stage: Effect | Pipeline): Pipeline => {
  if (isPipeline(stage)) {
    const stages: PipelineStage[] = stage.stages.map((s) => ({
      effect: s.effect,
...
export const pipe = (...stages: Array<Effect | Pipeline>): Pipeline => {
  ...
  for (const stage of stages) {
    if (isPipeline(stage)) {
      if (isAtPlaced(stage)) {
        ...
        if (stage.duration > maxEnd) maxEnd = stage.duration;
      } else {
