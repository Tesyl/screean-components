...
// six-logo · ink — the glTF particle cloud, inverted: black particles on a
// white surface.
//
// This is the dark-on-light twin of `sixLogo.ts`. The projection, cycling,
// and scatter logic are identical; the difference is entirely in the
// COMPOSITING, which is where dark-on-light usually breaks:
//
//   • The renderer defaults to additive "bloom" blending (dst = dst + src).
//     Additive can only BRIGHTEN — a black particle is (0,0,0) and adds
//     nothing, so it's invisible on any background. We pass `bloom: false`
//     to switch to source-over alpha, where pigment composites *over* and
//     *darkens* the surface beneath.
//   • We run in `portal: true` (transparent canvas) and paint the canvas
//     wrap white via CSS. Portal mode reveals that white on frame 0 — no
//     dark-flash while an opaque trail overlay converges to white.
//   • The palette is near-black charcoal (lit ≈ 0.06). The existing
//     depth-cued alpha then reads as near = rich black / far = soft gray,
//     a clean monochrome depth cue on white.
//
...
// Near-black charcoal. With sat 0 the hue is irrelevant; lit 0.06 keeps the
// softest sprite edges from vanishing into pure #000, reading as rich ink
// rather than a hard cutout. Shaped like the theme palette so it flows
// straight through `makeColor`.
...
// Compress the depth-cued alpha to a higher floor than the glow version.
// On a dark surface a far particle at alpha 46 still glowed; on white that
// same particle is ~18% gray and reads as washed out. Lifting the floor
// keeps the whole silhouette confidently dark while preserving depth order.
const ALPHA_FLOOR = 110; // far particles; near particles still reach 255
...
export const mount = (root: HTMLElement): (() => void) => {
  ...
  head.innerHTML = `
    <span class="doc-eyebrow">EXPERIMENT · 09</span>
    <h1>six-logo · ink — black particles on white</h1>
    <p>The gltf particle cloud inverted to dark-on-light. Additive "bloom" blending can only brighten, so black particles vanish on any surface; this runs source-over (pigment darkens the surface) over a white backdrop. Same projection, cycle, and scatter as six-logo — only the compositing changes. Click to scatter.</p>
  `;
  ...
  // Paint the canvas wrap white. Portal mode leaves the canvas transparent,
  // so this CSS surface is what shows through — present from frame 0, so no
  // dark flash before the cloud assembles.
  ...
  // Build the Stage in DARK-ON-LIGHT mode:
  //   • bloom: false   → source-over alpha; particles darken, not glow.
  //   • portal: true   → transparent canvas; the white wrap shows through.
  //   • trailAlpha 0.9 → near-instant clear to white each frame: crisp dots,
  //                       no wake. (Portal erase fades the canvas back to
  //                       transparent, revealing white.)
  const sg = new Stage({
    canvas,
    width: W,
    height: H,
    ...
    portal: true,
    bloom: false,
    particleSize: 1.1, // a touch larger than the glow version for presence on white
    trailAlpha: 0.9,
  });
  ...
  const rebuildCloud = (mesh: LoadedMesh, n: number): void => {
    ...
    // Base ink hues; the projection updater modulates alpha each frame.
  ...
  // so back-of-mesh particles read lighter (further away) on the white
  // surface while staying confidently dark.
  const projectFrame = (): void => {
    ...
    // Depth-cued alpha. Closest particles are full-ink (alpha 255), farthest
  ...
  loadGlb(sixLogoGlb)
    ...
    .catch((err) => {
      console.error('six-logo-ink: glb load failed', err);
